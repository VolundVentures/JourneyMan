  ...
  if (deploying) {
    return (
      <div className="flex flex-col items-center justify-center py-20 animate-fade-in">
        ...
        <h2 className="text-xl font-bold text-neutral-50 mb-8">Deploying {employeeName || 'Your AI Employee'}...</h2>
            ...
            return (
              <div key={i} className={cn(
                ...
                {i < deployStage ? (
                  ...
                  <Loader2 className="w-5 h-5 text-neutral-300 animate-spin shrink-0" />
                ) : (
                  <div className="w-5 h-5 rounded-full border border-neutral-700 shrink-0" />
                )}
                <span className={cn(
                  'text-sm',
                  i <= deployStage ? 'text-neutral-200' : 'text-neutral-600'
                )}>
                  {stage.label}
                </span>
              </div>
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-8 animate-fade-in">
      <div>
        ...
        <p className="mt-1 text-sm text-neutral-400">Deploy an AI employee with a real identity, personality, and purpose</p>
      </div>
          ...
          <div key={i} className="flex items-center gap-1">
            <button
              ...
              className={cn(
                ...
                i === step ? 'bg-neutral-800 text-neutral-50 border border-neutral-700' :
                i < step ? 'text-neutral-300 cursor-pointer hover:bg-neutral-800' : 'text-neutral-600'
              )}
            >
              <span className={cn(
                'w-5 h-5 rounded-full flex items-center justify-center text-[10px] font-bold',
                i === step ? 'bg-neutral-50 text-neutral-900' :
                i < step ? 'bg-neutral-600 text-white' : 'bg-neutral-800 text-neutral-500'
              )}>
                {i < step ? <Check className="w-3 h-3" /> : i + 1}
              </span>
              <span className="hidden md:inline">{s.label}</span>
            </button>
            {i < steps.length - 1 && <ChevronRight className="w-3.5 h-3.5 text-neutral-700" />}
          </div>
        ))}
      </div>
        ...
        <div className="space-y-6 animate-fade-in">
          ...
          <Card>
            ...
            <CardContent>
              <textarea
                ...
                className="w-full h-28 rounded-lg border border-neutral-700 bg-neutral-800/50 px-4 py-3 text-sm text-neutral-50 placeholder:text-neutral-500 focus:outline-none focus:ring-2 focus:ring-neutral-500 resize-none"
              />
          ...
          <div>
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-sm font-semibold text-neutral-400 uppercase tracking-wider">Or Choose a Template</h3>
                ...
                >
                  <div className="p-5 space-y-3">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <h3 className="font-semibold text-neutral-100 text-sm">{template.name}</h3>
                    ...
                    <div className="space-y-1">
                      {template.capabilities.slice(0, 3).map((cap) => (
                        <div key={cap.name} className="flex items-center gap-2 text-xs text-neutral-400">
                          <Check className="w-3 h-3 text-neutral-600 shrink-0" />
                          {cap.name}
                        </div>
                      ))}
                      {template.capabilities.length > 3 && (
                        <p className="text-[10px] text-neutral-600">+{template.capabilities.length - 3} more</p>
                      )}
                    </div>
                  </div>
                </div>
                    ...
                    {emojiOptions.map((emoji) => (
                      <button
                        ...
                        className={cn(
                          ...
                          selectedEmoji === emoji ? 'bg-neutral-700 ring-2 ring-neutral-500' : 'bg-neutral-800 hover:bg-neutral-700'
                        )}
                      >
                        {emoji}
                      </button>
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
                  {personalityTemplates.map((pt) => (
                    <button
                      ...
                      className="p-3 rounded-lg border border-neutral-800 hover:border-neutral-600 text-left transition-colors"
                    >
                      <p className="text-sm font-medium text-neutral-200">{pt.name}</p>
            ...
            <CardContent className="space-y-6">
              ...
              <div>
                ...
                <div className="space-y-2">
                  {selectedTemplate.defaultIntegrations.map((int) => (
                    <div key={int} className="flex items-center justify-between p-3 rounded-lg border border-neutral-800 bg-neutral-800/30">
                      <div className="flex items-center gap-2">
                        <Globe className="w-4 h-4 text-neutral-500" />
                        <span className="text-sm text-neutral-200 capitalize">{int.replace(/_/g, ' ')}</span>
                      </div>
                      <div className="flex items-center gap-2">
              ...
              <div>
                ...
                <select
                  ...
                  className="w-full max-w-xs rounded-lg border border-neutral-700 bg-neutral-800 px-3 py-2 text-sm text-neutral-200 focus:outline-none focus:ring-2 focus:ring-neutral-500"
                >
          ...
          <Card>
            ...
            <CardContent className="space-y-6">
              <div className="flex items-center gap-4 p-4 rounded-lg bg-neutral-800 border border-neutral-700">
                ...
                <div>
                  <h3 className="text-lg font-semibold text-neutral-100">{employeeName || 'Unnamed Employee'}</h3>
              ...
              <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
                {[
                  ...
                  <div key={i} className="flex items-center gap-2.5 p-3 rounded-lg bg-neutral-800/50">
                    <item.icon className="w-4 h-4 text-neutral-500" />
                    <div>
                      <p className="text-[10px] text-neutral-600">{item.label}</p>
                      <p className="text-sm text-neutral-200 capitalize">{item.value}</p>
                    </div>
                  </div>
                ))}
              </div>
                ...
                <div className="grid grid-cols-3 sm:grid-cols-6 gap-3">
                  {Object.entries(personality).map(([key, val]) => (
                    <div key={key} className="text-center">
                      <div className="text-lg font-bold text-neutral-200">{val}</div>
